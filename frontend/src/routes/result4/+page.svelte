  ...
  function processFile(file) {
    ...
    reader.onload = function(event) {
      ...
      for (let i=0; i < fileRows.length; i++) {
        ...
        if (removecolumnidxrow.filter(element => true === element).length >= parseInt((removecolumnidxrow.length)/2)) {
    ...
    <div class="mt-10">
      <div class="w-full px-10 rounded-lg">
        ...
        {#if preview}
          ...
          <div class="mt-2 overflow-x-auto p-2 m-2">
                ...
                {#each fileRows.slice(0, fileRows.length) as row, rowIndex}
                  {#if rowIndex === 0}
                    ...
                    <tr>
                      ...
                      {/if}
                      {#each row.slice(0, 10) as cell, cellIndex}
                        <td class="mr-2 text-center text-sm py-2 px-5">
                          {cell}
                        </td>
                      {/each}
                    </tr>
